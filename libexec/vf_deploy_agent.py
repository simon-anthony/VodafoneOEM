...
parser = argparse.ArgumentParser(
    ...
    epilog='Like submit_add_host() but with properties. Edit the files @PKGDATADIR@')
...
group_oms.add_argument('-r', '--region', choices=config_region.sections(), help='region to lookup section in @PKGDATADIR@/region.ini')

# node
config_node = ConfigParser.ConfigParser()
config_node.read('@PKGDATADIR@/node.ini')
parser.add_argument('-n', '--node', required=True, choices=config_node.sections(), help='node section in @PKGDATADIR@/node.ini, should provide settings for \'credential_name\', \'credential_owner\', \'installation_base_directory\', \'instance_directory\'')
...
#group_oms.add_argument('-l', '--lifecycle', choices=config_props.options('Lifecycle Status'), help='lifecycle status @PKGDATADIR@/properties.ini')
group_oms.add_argument('-l', '--lifecycle', choices=config_props.options('Lifecycle Status'), metavar='<value>', help='lifecycle status @PKGDATADIR@/properties.ini')
